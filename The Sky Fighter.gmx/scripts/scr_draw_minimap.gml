...
with(obj_parent_fighter)
{
   var fighter_minimap_pos_x = x * scale_factor + left_corner_x;
   var fighter_minimap_pos_y = y * scale_factor + left_corner_y;
   
   if(object_get_parent(self.object_index) == obj_enemy){
    draw_set_colour(c_red);}
   else{
    draw_set_colour(c_yellow);}
    
   draw_triangle(lengthdir_x(10, direction) + fighter_minimap_pos_x, 
        lengthdir_y(15, direction) + fighter_minimap_pos_y,
        lengthdir_x(5, direction + 90) + fighter_minimap_pos_x,
        lengthdir_y(5, direction + 90) + fighter_minimap_pos_y,
        lengthdir_x(5, direction - 90) + fighter_minimap_pos_x,
        lengthdir_y(5, direction - 90) + fighter_minimap_pos_y, false);
}
